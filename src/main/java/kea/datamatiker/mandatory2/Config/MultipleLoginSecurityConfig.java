package kea.datamatiker.mandatory2.Config;
...
public class MultipleLoginSecurityConfig {
    ...
    public UserDetailsService userDetailsService() throws Exception {
        ...
        manager.createUser(User.withUsername("student").password(encoder().encode("studentPass")).roles("STUDENT").build());
    ...
    public static class App1ConfigurationAdapter extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity http) throws Exception {
            http.antMatcher("/admin*").authorizeRequests().anyRequest().hasRole("ADMIN")
                    // log in
                    .and().formLogin().loginPage("/login").loginProcessingUrl("/admin_login").failureUrl("/loginAdmin?error=loginError").defaultSuccessUrl("/adminstration/index")
                    // logout
    ...
    public static class App2ConfigurationAdapter extends WebSecurityConfigurerAdapter {
        ...
        @Override
        protected void configure(AuthenticationManagerBuilder auth) throws Exception {
            auth.inMemoryAuthentication().withUser("student").password(encoder().encode("student")).roles("STUDENT");
        }
